# This is the original file in which we're working on a live project:
...
# "local_server_uri" : mysql://root:@localhost/the_secret
...
app.config.update(
    ...
    MAIL_PORT = 465,
    MAIL_USE_SSN = True, # This is for the encryption purpose.
...
if (local_server):
    app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:@localhost/the_secret'
# This is the initialization of the 'db' variable.
else:
    app.config['SQLALCHEMY_DATABASE_URI'] = 'prod_server_uri'
db = SQLAlchemy(app)


# This class defines the database tables.
...
def contact():
    # Here we're creating the content for filling the contact form.
    # We'll use the POST_request here.
    if (request.method=='POST'):
...
def dashboard():
    ...
    # if user is already logged in then, give the access of the website:
    if ('user' in session and session['user'] == params['admin_user']):
...
def post_route(post_slug):
    # By passing variable post_slug, to post function, we can directly access that variable from this function.
